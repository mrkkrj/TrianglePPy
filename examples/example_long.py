...
for f in faces:
    ...
    pointApx = f.apex() # then: apex_point = pointApx[0]
    pointOrg, _ = f.org()

    # or get just the index/point
...
points = []
try:
    d.read_points("./triangulation_points.node", points)
    if output_level != DebugOutputLevel.Nothing:
        print(f"Read points from a .node file, count: {len(points)}")
